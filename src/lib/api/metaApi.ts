...
class RequestQueue {
  ...
  constructor(concurrency: number = 2, delayMs: number = 1500) {
...
export async function fetchAllCampaigns(
  config: MetaConfig,
  after?: string,
  limit: number = META_API.PAGE_SIZE
  ...
  return metaQueue.add(() => 
    callMetaApi<PaginatedResponse<MetaCampaign>>(
...
export async function fetchAllAdSets(
  ...
  const result = await metaQueue.add(() => 
    callMetaApi<PaginatedResponse<MetaAdSet>>(
      `${cleanActId}/adsets`,
      params,
      config
    )
...
export async function fetchAllAds(
  ...
  return metaQueue.add(() => 
    callMetaApi<PaginatedResponse<MetaAd>>(
      `${cleanActId}/ads`,
      params,
      config
    )
