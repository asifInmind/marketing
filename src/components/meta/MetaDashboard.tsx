...
import { ShopifyConnectModal } from '../shopify/ShopifyConnectModal';
...
export function MetaDashboard({ accessToken, accountId }: MetaDashboardProps) {
  const [activeTab, setActiveTab] = useState<'campaigns' | 'adSets' | 'ads' | 'shopify'>('campaigns');
  ...
  // Shopify table filters and details slide-over states
  const [shopifyAdFilter, setShopifyAdFilter] = useState<'all' | 'running'>('running');
  const [shopifyPerfSort, setShopifyPerfSort] = useState<'best' | 'bad'>('best');
  const [selectedProductPerformance, setSelectedProductPerformance] = useState<any | null>(null);
  ...
  // Shopify Dashboard Integration Hook
  const {
    shopifyUrl,
    ...
    loadMoreProducts,
  } = useShopifyDashboard(ads);

  // Compute filtered & sorted product list for Shopify Product Table
  const filteredAndSortedProducts = useMemo(() => {
    let items = [...productPerformance];

    // Filter 1: Scope
    if (shopifyAdFilter === 'running') {
      items = items.filter(item => item.adSpend > 0 || (item.matchedAds && item.matchedAds.length > 0));
    }

    // Filter 2: Sort (only enabled/active when Filter 1 is 'running')
    if (shopifyAdFilter === 'running') {
      items.sort((a, b) => {
        if (shopifyPerfSort === 'best') {
          return b.trueROAS - a.trueROAS; // High to Low ROAS
        } else {
          return a.trueROAS - b.trueROAS; // Low to High ROAS
        }
      });
    }

    return items;
  }, [productPerformance, shopifyAdFilter, shopifyPerfSort]);

  // Detect Shopify OAuth callback parameters from URL
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const shopToken = urlParams.get('shopify_token');
    const shopUrl = urlParams.get('shopify_url');

    if (shopToken && shopUrl) {
      connectManual(shopUrl, shopToken);

      // Clean URL parameters
      const newUrl = new URL(window.location.href);
      newUrl.searchParams.delete('shopify_token');
      newUrl.searchParams.delete('shopify_url');
  ...
  const formatShopifyCurrency = useCallback((amount: number) => {
    try {
  ...
  const handleTabChange = (tab: 'campaigns' | 'adSets' | 'ads' | 'shopify') => {
    setActiveTab(tab);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <div className="relative">
                        <select
                          value={shopifyAdFilter}
                          onChange={(e) => {
                            const val = e.target.value as 'all' | 'running';
                            setShopifyAdFilter(val);
                            if (val === 'all') {
                              setShopifyPerfSort('best');
                            }
                          }}
                        ...
                        >
                          <option value="running">Running Ads Only</option>
                      ...
                      {/* Filter 2: Performance Sort */}
                      <div className="relative">
                        <select
                          value={shopifyPerfSort}
                          disabled={shopifyAdFilter !== 'running'}
                          onChange={(e) => setShopifyPerfSort(e.target.value as 'best' | 'bad')}
                          className="appearance-none pl-9 pr-8 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-xs font-semibold text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 cursor-pointer transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          ...
                          {filteredAndSortedProducts.length === 0 ? (
                            ...
                            filteredAndSortedProducts.map(item => (
                              <tr key={item.productId} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/10">
                                ...
                                <td className="p-4 text-right font-semibold text-slate-500 dark:text-slate-400">${item.adSpend.toFixed(2)}</td>
      ...
      <ShopifyConnectModal
        ...
        onConnectOauth={(shopUrl) => connectOauth(shopUrl, accountId, accessToken)}
        onConnectManual={connectManual}
        loading={shopifyLoading}
      />

      {/* Slide-over Ad Performance Details Panel */}
      {selectedProductPerformance && (
        ...
        >
          ...
          {/* Drawer Panel (80% width with animation) */}
          <div
            className="w-[80%] bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 shadow-2xl flex flex-col h-full z-50 transform translate-x-full animate-slide-in"
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-950/30 rounded-xl border border-slate-100 dark:border-slate-800/40 shadow-sm">
                  ...
                  <div className="text-2xl font-bold text-slate-900 dark:text-white mt-1">
                    ${selectedProductPerformance.adSpend.toFixed(2)}
                  </div>
                  <div className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-950/30 rounded-xl border border-slate-100 dark:border-slate-800/40 col-span-2 md:col-span-1 shadow-sm">
                  ...
                  <div className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">
                    Avg. CPC: ${selectedProductPerformance.adSpend > 0 && selectedProductPerformance.adClicks > 0 ? (selectedProductPerformance.adSpend / selectedProductPerformance.adClicks).toFixed(2) : '—'}
                  </div>
                </div>
              </div>
                ...
                {(!selectedProductPerformance.matchedAds || selectedProductPerformance.matchedAds.length === 0) ? (
                  ...
                  <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
                    <table className="w-full text-left text-xs border-collapse">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-slate-700 dark:text-slate-300">
                        {selectedProductPerformance.matchedAds.map((ad: any) => {
                          ...
                          return (
                            <tr key={ad.id} className="hover:bg-slate-50/30 dark:hover:bg-slate-950/5">
                              ...
                              <td className="p-3 text-right font-medium">${ad.cost?.toFixed(2)}</td>
              ...
              {/* Product Variant Details & AI Recommendations */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        {(() => {
                          ...
                          return rawProduct.variants.map((v) => (
                            <tr key={v.id}>
                              ...
                              <td className="py-2.5 text-right font-medium">${parseFloat(v.price).toFixed(2)}</td>
                ...
                {/* AI Performance Recommendation Panel */}
                <div className="border border-slate-200 dark:border-slate-800 rounded-xl p-5 space-y-4 shadow-sm bg-white dark:bg-slate-900">
                  <h4 className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider">
